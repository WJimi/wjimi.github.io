...
`information_schema` 是MySQL等数据库中的**系统数据库**，用于存储数据库的元数据（数据库名、表名、列名、权限等）**不存储用户业务数据**。
...
POST 请求中注释常用 `#`，而不是 `--+`。  
因为 `#` 不在 URL 里，不会被浏览器当作锚点截断；而 `--+` 中的 `+` 是 URL 中代表空格的编码，POST body 中通常不涉及这种处理。

**`or 1=1` 的核心作用就是：让整个 `WHERE` 条件永远为真。即使原本查询条件（比如 `uname='1'`）不匹配任何记录，数据库也会因为 `or 1=1` 而返回表中的某一行数据（通常是第一条）。**

**当界面符合预期，就意味着 payload 成功的可能更接近目标。**

1. **验证注入点存在**  
    正常输入 `uname="1"` 可能返回空或错误；加上 `or 1=1` 后页面突然正常登录或显示数据 → 说明你的输入影响了原 SQL 逻辑，**注入点确认存在**。
    
2. **绕过第一道防线**  
    登录框场景下，这个 payload 可以直接进入后台（以第一个用户身份），属于高风险漏洞。
    
3. **为下一步数据提取铺路**  
    一旦验证 `or 1=1` 有效，就可以尝试：
...
另外，当知道 `or 1=1` 符合预期时，可换用精细工具；盲注语句中也可用其他恒真条件替换 `1=1`。
